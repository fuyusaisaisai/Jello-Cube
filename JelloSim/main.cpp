...
//World theWorld("worlds/cylinders2.xml");
...
//void rayHit(vec3 m_start,vec3 m_end);
...
void rayHit(vec3 m_start,vec3 m_end)
{
	JelloMesh::ParticleGrid& g = theJello.m_vparticles;

	vec3 normalRay = (m_end-m_start).Normalize();

	double min_length = 100000.0;
	int rows = theJello.GetGridRows();
	int cols = theJello.GetGridCols();
	int stacks = theJello.GetGridStacks();
	int hit_index;
	int j =0;
	for (int i = 0; i < rows+1; i++)
    {
		for (int j = 0; j < cols+1; j++)
        {
            for (int k = 0; k < stacks+1; k++)
            {
				...
				{
					JelloMesh::Particle& p = theJello.GetParticle(g, i,j,k);
